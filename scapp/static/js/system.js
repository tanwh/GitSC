...
function addroles(){    
    ...
    if(select.selectedIndex!=-1){//选取值不为空时执行
        for(j=0;j<roles.options.length;j++){//获取已选职能中的值         
            if(options=roles.options[j].text==select.options[select.selectedIndex].text){//判断已选职能中是否已经包含该值
                    alert("该职能已选择，不能重复选择！！");
                    return
            }           
        }       
        $("<option>"+select.options[select.selectedIndex].text+"</option>").appendTo("#roles"); //添加option项
    }
    else
        alert("请先选择职能，然后再添加！！");    
}
